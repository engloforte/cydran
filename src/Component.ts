...
class UtilityComponentFactoryImpl implements ComponentFactory {
	...
	constructor(module: Module, template: string, prefix: string, parentId: string, parentModelFn: () => any) {
		this.module = module;
	...
	public createForItem(item: any): Nestable {
		return this.createForUtility(() => item);
	}

	public createForUtility(valueFn: () => any): Nestable {
...
class EmbeddedComponentFactoryImpl implements ComponentFactory {
	...
	public createForUtility(valueFn: () => any): Nestable {
		...
		component.message(INTERNAL_DIRECT_CHANNEL_NAME, "setItemFn", valueFn);
	...
	public createForItem(item: any): Nestable {
		const component: Nestable = this.createForUtility(() => item);
...
class Each extends AbstractElementMediator<any[], HTMLElement, Params> {
	...
	public wire(): void {
		...
		for (let i = 0; i < children.length; i++) {
			...
			switch (type) {
				case "empty":
					this.empty = this.createFactory(template).createForUtility(valueFn);
					break;

				case "first":
					this.first = this.createFactory(template).createForUtility(valueFn);
					break;

				case "after":
					this.last = this.createFactory(template).createForUtility(valueFn);
					break;

				case "alt":
					const expression: string = this.getExtractor().extract(template, "test");
					this.alternatives.push({
						factory: this.createFactory(template),
				...
				case "item":
					this.itemFactory = this.createFactory(template);
					break;
			}
		}
	...
	private create(item: any): Nestable {
		...
		return factory.createForItem(item);
	}

	private createFactory(template: HTMLTemplateElement): ComponentFactory {
		...
		return (hasComponentId)
			...
			: new UtilityComponentFactoryImpl(
				this.getModule(),
